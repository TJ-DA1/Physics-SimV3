...
class Element:
    def initiate(self):
        ...
        self.deglabel = pygame_gui.elements.UILabel(relative_rect=pygame.Rect((0, 40), (200, 20)), text=f"Gravity angle: {ctx.deg - 90}", manager=ctx.manager)
        self.degslider = pygame_gui.elements.UIHorizontalSlider(relative_rect=pygame.Rect((0, 60), (200, 20)), start_value=ctx.deg - 90, value_range=(-180, 180), manager=ctx.manager)

        self.restlabel = pygame_gui.elements.UILabel(relative_rect=pygame.Rect((0, 80), (200, 20)), text=f"Restitution: {ctx.restitution}", manager=ctx.manager)
        self.restslider = pygame_gui.elements.UIHorizontalSlider(relative_rect=pygame.Rect((0, 100), (200, 20)), start_value=ctx.restitution * 10, value_range=(0, 10), manager=ctx.manager)

        self.friclabel = pygame_gui.elements.UILabel(relative_rect=pygame.Rect((0, 120), (200, 20)), text=f"Friction: {ctx.friction}", manager=ctx.manager)
        self.fricslider = pygame_gui.elements.UIHorizontalSlider(relative_rect=pygame.Rect((0, 140), (200, 20)), start_value=ctx.friction * 10, value_range=(0, 10), manager=ctx.manager)
