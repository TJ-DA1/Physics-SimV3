...
class CollHandler:
    ...
    def checkcircle(self, b1, b2): #Checks the distance against sum of radii
    ...
    def checkcirclesquare(self, b, s):
        ...
        if math.dist((b.x, b.y), (s.x, s.y)) > (max(s.sizex, s.sizey) * 0.5 * (2 ** (1 / 2))) + b.radius: #Heuristic check
            return
    ...
    def overlapcircle(self, b1, b2): #Finds overlap and uses trig to move balls away from each other
    ...
    def collcircle(self, b1, b2):
        diffx, diffy = b2.x - b1.x, b2.y - b1.y
        collangle = math.atan2(diffy, diffx)
        b1norm = (b1.dx * math.cos(collangle)) + (b1.dy * math.sin(collangle)) #Split ball velocity into tangential and normal components
        ...
        b1.dx = (b1norm * math.cos(collangle)) - (b1tan * math.sin(collangle)) #Combine normal and tangential into X and Y
    ...
    def collcirclesquare(self, b, s, side, dist):
        ...
        b1norm = (b.dx * math.cos(collangle)) + (b.dy * math.sin(collangle)) #Split velocity into tangential and normal
        b1tan = (-1 * b.dx * math.sin(collangle)) + (b.dy * math.cos(collangle))
        rest = ctx.restitution ** (1 / ctx.passes) #Apply energy loss - reduced for higher passes due to repeated collisions
        fric = ctx.friction ** (1 / ctx.passes)
        b.dx = -rest * b1norm * math.cos(collangle) - b1tan * math.sin(collangle) * fric #Combine into X and Y components
    ...
    def colledgecase(self, b1, b2):
        diffx, diffy = b2.x - b1.x, b2.y - b1.y
        collangle = math.atan2(diffy, diffx)
        b1norm = (b1.dx * math.cos(collangle)) + (b1.dy * math.sin(collangle)) #Split velocity into tangential and normal
        b1tan = (-1 * b1.dx * math.sin(collangle)) + (b1.dy * math.cos(collangle))
        b1.dx = -ctx.restitution * b1norm * math.cos(collangle) - b1tan * math.sin(collangle) #Combine into X and Y components
        b1.dy = -ctx.restitution * b1norm * math.sin(collangle) + b1tan * math.cos(collangle)

    def overlapedgecase(self, b1, b2):
        diffx, diffy = b2.x - b1.x, b2.y - b1.y #Same as normal overlap but only pushes one ball by full overlap
        distance = math.dist((b1.x, b1.y), (b2.x, b2.y))
        ...
        b1.x -= (overlap * nx) #Push ball away from unaffected ball
        b1.y -= (overlap * ny)